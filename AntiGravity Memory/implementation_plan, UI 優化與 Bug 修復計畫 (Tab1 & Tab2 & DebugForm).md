...
- **DebugForm 問題 (分組排序)**: 目前按函數分組導致時間序混亂。應改為「按使用者的一次操作」分組，例如點選目錄樹後的連動過程應屬於同一個群組。
...
- **`AddMessage3`**: 
    - 引入「Session」概念。偵測到 `_Click`, `_AfterSelect`, `_Shown`, `_Load` 等入口函數時，重設 `_activeSessionName`。
    - 所有訊息優先歸類於當前的 `_activeSessionName` 群組下。
    - **排序修復**: 取消按函數名稱強行分組的做法，改為基於時間序的線性顯示，或僅在「大型流程入口」時才切換群組。
