# 修復 Splitter 殘影問題 Walkthrough

本次修改解決了 `SplitContainer` 在雙擊分隔線切換寬度時，`ListView` 邊緣可能殘留舊畫面雜訊的問題。

## 修改內容
### Form1.vb
- **SplitterToggle 方法優化**：
    - 在呼叫 `WM_SETREDRAW` (False) 凍結重繪**之前**，先對 `sc.Panel2` 執行一次 `Invalidate()`。這會確保 Windows 知道舊有的顯示區域已經失效，在接下來的重繪循環中會正確擦除背景。
    - 在還原重繪後，使用了 `sc.Panel2.Invalidate(True)`。這裡的 `True` 非常關鍵，它會強制所有子控制項（包含 ListView）及其背景進行重繪。
    - **額外補強**：針對活動中的 `ListView`（如果是 `OwnerDraw` 模式），額外執行一次 `lv.Invalidate()` 與 `lv.Update()`，確保同步更新，徹底消除雜訊。

## 驗證結果
- **邏輯複檢**：確認 `Invalidate()` 與 `Update()` 的順序正確，且在凍結前標記無效區域是解決「舊畫面殘留」的標準 WinForms 做法。
- **程式碼完整性**：保留了所有原始註解，並依照要求標註了 `by Gemini 3 Flash, 2026/05/09`。
...
#Region "  ├ 滑鼠 & 鍵盤操作事件"
    ...
    Private Sub SplitterToggle(sc As SplitContainer)
        ...
        Finally
            ...
            sc.Panel2.Invalidate(True)
            sc.Panel2.Update()          ' 同步執行 WM_PAINT，確保畫面立即更新

            ' 若內部有 ListView，針對 OwnerDraw 模式額外補強一次全量重繪，消除邊界雜訊
            Dim lv = GetCurrentLv()
            If lv IsNot Nothing AndAlso lv.OwnerDraw Then
                lv.Invalidate()
                lv.Update()
            End If
        End Try
