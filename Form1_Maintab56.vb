...
#Region "  ├ Layer2 流程協調層"
    ...
    Private Sub HandleLv5Delete(lv As ListView)
        ...
        '''   殘留的單封孤兒群不在此清除；待使用者按 F5(RefreshLviAllItems 末端 RenderLv5Group 丟棄 Count<=1 群並清紅字) 或重新搜尋時才整理。
        ''' </summary>
        _dbg("開始")
...
#Region "  ├ Fuzzy 模糊比對專用區塊 (SimHash + bigram Jaccard)"
    ...
    Private Const MIN_BIGRAM_FOR_FUZZY As Integer = 20
    ...
    Private Async Function PreComputeFuzzySimHashAsync(mails As List(Of MailItemInfo), progress As IProgress(Of ProgressReport), cToken As CancellationToken) As Task
        ...
        Dim totalBodyChars As Long = 0   ' 探針(量測用,可移除): 累計 body 字元數，估算全快取記憶體footprint
        Dim batch As New List(Of (EntryID As String, SimHash As Long, BigramCount As Integer))(512)
        ...
        For i As Integer = 0 To todo.Count - 1
            ...
            ' 2026/06/18 探針(可移除)
            Dim body As String = GetMailBodyL3(id) : totalBodyChars += body.Length : Dim setB = BuildBigramSet(body)
            ...
            If batch.Count >= 500 Then SaveSimHashBatch(batch) : batch.Clear()   ' 分批 flush，兼作斷點(中斷後可續算)
        ...
        Next
        ' 2026/06/18 探針(可移除)
        _dbg("Body量測", $"S3 讀 {todo.Count} 封, body 累計 {totalBodyChars:N0} 字元 ≈ {totalBodyChars * 2.0 / 1048576.0:F1} MB(純UTF-16) + 約 {todo.Count * 26.0 / 1048576.0:F1} MB(字串物件開銷)。此即全進 _cacheMailBody 的概估量")

        If batch.Count > 0 Then SaveSimHashBatch(batch)
    End Function
    Private Async Function GenerateFuzzyCandidatePairs(mails As List(Of MailItemInfo), targetT As Double, cToken As CancellationToken) As Task(Of List(Of (A As MailItemInfo, B As MailItemInfo)))
        ...
        Return Await Task.Run(
            Function() As List(Of (A As MailItemInfo, B As MailItemInfo))
                ...
                Dim items = mails.
                    Where(Function(m) _cacheSimHash.ContainsKey(m.EntryID) AndAlso _cacheSimHash(m.EntryID).BigramCount >= MIN_BIGRAM_FOR_FUZZY).
                ...
                Dim probePairs As Long = 0, probeCand As Long = 0   ' D6 探針
                For i As Integer = 0 To items.Count - 1
                    ...
                    For j As Integer = i + 1 To items.Count - 1
                        ...
                        probePairs += 1
                        If GetHammingDistance(items(i).SH, items(j).SH) <= hThr Then
                            result.Add((items(i).Mail, items(j).Mail)) : probeCand += 1
                        End If
                    Next
                Next
                _dbg("S4候選", $"視窗內比對 {probePairs} 對 → Hamming(≤{hThr}) 過關 {probeCand} 對 (待 S5 Jaccard 精算)")
                Return result
            End Function, cToken)
    End Function
    Private Async Function FilterCandidatesByJaccardAsync(candidates As List(Of (A As MailItemInfo, B As MailItemInfo)), targetT As Double, progress As IProgress(Of ProgressReport), cToken As CancellationToken) _
        ...
        For k As Integer = 0 To uniqueIds.Count - 1
            ...
            If (k And 15) = 0 Then
                ...
                progress?.Report(New ProgressReport With {.Message = $"讀取候選內文: {k + 1}/{uniqueIds.Count} ({eta.Speed:F0} 個/秒{eta.EtaString})"})
                Await Task.Delay(1, cToken)
            End If
        Next
        ...
        Dim pairs = Await Task.Run(
            Function() As List(Of (A As MailItemInfo, B As MailItemInfo, Score As Double))
                ...
                For Each p In candidates
                    'Dim s As Double = BigramJaccardSimilarity(sets(p.A.EntryID), sets(p.B.EntryID))   ' size 1/T 界線 S4 已保證，此處不重複早退
                    'If s >= targetT Then r.Add((p.A, p.B, s))

                    ' Q1-C 2026/06/18 by Simon/Claude Opus 4.8 修改上面二行:
                    '   先取交集絕對數，不足門檻直接淘汰(擋短信比例 100% 假陽性)；達標再由 inter 導出 Jaccard，免重算交集
                    Dim setA = sets(p.A.EntryID), setB = sets(p.B.EntryID)
                    Dim inter As Integer = BigramIntersectionCount(setA, setB)
                    If inter < MIN_SHARED_BIGRAM_FOR_FUZZY Then Continue For
            ...
            Select Case result
                ...
                Case DialogResult.No
                    If MessageBox.Show("【安全提示】這將把目前的 SSD 快取檔更名備份 ( .zip) 並重新建立空白資料表。" & vbCrLf & "這可以解決 Schema 不相容問題且具備救援機制，確定嗎？", "重置 SSD 快取", MessageBoxButtons.OKCancel, MessageBoxIcon.Information) = DialogResult.OK Then
                        ...
                        If chkClearSimHash.Checked Then DeleteSimDatabase() ' 2026/06/17 by Simon/Claude Opus 4.8: 勾選 chkClearSimHash 時連同 SSD 一併清除 SimHash 獨立 db (DeleteSimDatabase 內含關連線/刪檔/清記憶體/重建空表)
                        ProgressBar2.Text = "已完成：SSD 資料庫已備份並重新初始化" & If(chkClearSimHash.Checked, " (含 SimHash 清除)", "")
                ...
                Case DialogResult.Retry
                    If MessageBox.Show("確定要清除記憶體並備份重置 SSD 快取嗎？", "最後確認", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning) = DialogResult.OK Then
                        ClearMemoryCachesCore()
                        Await ZipAndRebuildDB()
                        If chkClearSimHash.Checked Then DeleteSimDatabase() ' 2026/06/17 by Simon/Claude Opus 4.8: 勾選 chkClearSimHash 時連同 SSD 一併清除 SimHash 獨立 db
                        ProgressBar2.Text = "已完成：記憶體與 SSD 快取已全數歸零 (舊 SSD 檔已備份)" & If(chkClearSimHash.Checked, " + SimHash 已清除", "")
