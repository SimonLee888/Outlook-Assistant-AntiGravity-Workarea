...
本檔轉交「下一個對話」執行**兩件待辦（皆尚未套用）**：
- **Part 1（待套用）**：`RenewCacheToDB` 死列(失效 entryID) purge 修復 — 檢查結論 + 完整 task list + 逐處 old→new changeset，照著套即可。
- **Part 2（待規劃執行）**：DB 切分 — 把 `attach_filenames` 從 `OLAcache.db` 搬入 `OLAsimhash.db`（與 SimHash 表同檔），含完整接觸點地圖、耦合點與待拍板決策。

> **這兩件事的程式碼分析、修改細項都已在本檔內，但都還沒套進專案。** 下一個對話要負責「實際套用 Part 1 + 規劃執行 Part 2」。

兩件事**強耦合且有順序**：Part 1 的 purge 會在 `DbPurgeFolderMailRows` 與 `CleanupOrphanPath` 兩處 DELETE `attach_filenames`；Part 2 把該表搬走後，這兩處 DELETE 必須改連 `_dbSim`。**執行順序：先完整套用 Part 1（attach_filenames DELETE 落在 `_db`、可獨立編譯驗證）→ 驗證後再做 Part 2（含 re-point 那兩處 DELETE）**。理由見 §2.4。
（替代做法：兩件一次合併、Part 1 的 attach_filenames DELETE 直接寫 `_dbSim`，省一次改寫；但較難分段驗證，**建議仍照上面分兩步**。）

---

# Part 1 — RenewCacheToDB 死列 purge 修復【待套用 — 下一對話執行】
...
| # | 位置（搜尋錨點） | 動作 | 狀態 |
|---|---|---|---|
| 1 | `RenewCacheToDB` 檔頭 `' 流程：` | 改寫流程/設計邊界，補 06/20 註解（保留 04/09 等舊署名） | 待套用 |
| 2 | 狀況 A `_cacheBasicMailInfo.TryRemove(fPath` 之後 | 補清 attach/month 記憶體 + 呼叫 `DbPurgeFolderMailRows` | 待套用 |
| 3 | step 7 `For Each ancestor In GetAncestors(p)` | 改 `GetAncestors(p).Prepend(p)`、`ancestor`→`node`（補入自身 All 聚合） | 待套用 |
| 4 | `Private Async Function RenewAttachMailList`（死碼） | 整段刪除，原位置改放新 helper `DbPurgeFolderMailRows` | 待套用 |
| 5 | `SaveBasicMailInfoInner` 的 `_dbg("開始")` 後 | 加寫入邊界 purge（INSERT 前先刪 basic_maillist 該夾舊列） | 待套用 |
| 6 | `SaveAttachMailListInner` 的 `_dbg("開始")` 後 | 加寫入邊界 purge（attach_maillist） | 待套用 |
| 7a/7b | `SaveMonthCountsInner` 的 `Using cmd...sql` / `For Each mo In kvp.Value` | 加 delCmd、逐 `(folder_hash, year)` 先刪後插 | 待套用 |
| — | 檔頭索引註解 line 21/22 | RenewCacheToDB 改「精確打擊」描述；刪 `RenewAttachMailList` 索引行 | 待套用 |
...
## 1.3 可重套用 changeset（下一對話照此套進專案）

> 以下為各處 old→new，在 Visual Studio 手動套用。署名日期統一用 `2026/06/20`（與規劃時對話一致；如要改 06/21 自行全域替換即可）。**全部尚未套用。**
...
> 因此**順序固定（同一對話內連續執行）**：先完整套用 Part 1（§1.3 全部 changeset，attach_filenames DELETE 落在 `_db`，先讓專案能編譯/驗證）→ 再做 Part 2（把這兩處 + 寫入/讀取/建表 re-point 到 `_dbSim`）。若顛倒順序，Part 1 的 changeset 會把 DELETE 又寫回 `_db`，得再改一次。
> （若要省一次改寫，可在套 Part 1 時就把這兩處 attach_filenames DELETE 直接寫成 `_dbSim` 獨立交易版本；但這等於把 Part 2 的決策提前，**建議仍分兩步、先驗證 Part 1**。）
...
「這份 memory 裡有**兩件待辦、都還沒套進專案**：

**先做 Part 1**：照 §1.3 的 8 處 changeset，把 `RenewCacheToDB` 死列 purge 修復套進 `Module_SQLite2.vb`（建議先做 #4 新增 `DbPurgeFolderMailRows`，#2 才有對象可呼叫）。套完先確認能編譯、行為正常。

**再做 Part 2**：把 `attach_filenames` 從 `OLAcache.db` 搬進 `OLAsimhash.db`（與 `mail_simhash` 同檔）。請先讀 §2.3 接觸點地圖、§2.4 與 Part 1 的耦合點（Part 1 在 `DbPurgeFolderMailRows` 與 `CleanupOrphanPath` c3 兩處 DELETE 了 attach_filenames，搬移後要 re-point 到 `_dbSim`）、§2.6 順序。**Part 2 動手前，先就 §2.5 五個決策列選項給我確認，不要直接改。** 所有改動集中在 `Module_SQLite2.vb`（少數 UI 字串在 `Form1_Maintab56.vb`）。」

---

## 附：本次規劃新增/變更的符號速查（皆待套用）
