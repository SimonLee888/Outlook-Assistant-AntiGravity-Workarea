...
#Region "■ 07 失焦 / 得焦"
    ...
    Protected Overrides Sub OnGotFocus(e As EventArgs)
        ...
        '    但它造成副作用：ExpandTreeToDefaultInbox 設好 inbox 後，
        '    多個 Await Task.Yield 之間若 _selectedNodes 被清空，
        '    BeginInvoke(SimTree2.Focus()) 觸發 OnGotFocus 時會覆蓋 inbox 選取。
        '    改為：「第一次按鍵時若無選取則選 TopNode」，由 OnKeyDown 開頭處理，
        '    效果相同但不受 Await 競爭條件影響。
        MyBase.OnGotFocus(e)
...
#Region "■ 10 公共輔助方法（供 Form1 呼叫）"
    ...
    Public Sub AddSelectedNode(node As TreeNode)
        ' AddSelectedNode：從外部新增一個選定節點，不清除其他選取，不觸發 AfterSelect
    ...
    Public Sub SetSelectedNode(node As TreeNode)
        ' SetSelectedNode：等同 SelectedNode = node（保留相容舊版呼叫，不觸發 AfterSelect）
        SelectSingleNode(node)
    End Sub
#End Region
