...
Module moduleStore
    ...
    Private lblStatus1 As Object
    Private lblStatus2 As Object
    ...
    Private Sub LoadSubFolders(folder As Outlook.Folder, nodes As TreeNodeCollection, tvwTarget As TreeView)
        ...
        '                          tvwTarget.EndUpdate()
        '                          lblStatus2.Text = "載入所有資料夾花費了 " & sw0.Elapsed.TotalSeconds.ToString("0.00") & " 秒。"
        '                      End Sub)
    End Sub
    ...
    Private Async Sub AddSubFoldersToTreeViewByBgWorker(folder As Outlook.Folder, nodes As TreeNodeCollection, tvwTarget As TreeView)
        ...
        ''''                       tvwTarget.Invoke(Sub()
        ''''                                            lblStatus1.Text = $"正在載入: {TotalFolderCount} ({TotalFolderCount / sw0.Elapsed.TotalSeconds:###,##0}/sec) {folder.Name}資料夾"
        ''''                                        End Sub)
        ''''                   End While
        ''''               End Sub)
        '''Await loadTask
        ...
        '                                While Not loadTask.IsCompleted
        '                                    Await Task.Delay(250) ' 延遲250毫秒
        '                                    tvwTarget.Invoke(Sub() lblStatus1.Text = $"正在載入: {TotalFolderCount} ({Math.Round(TotalFolderCount / sw0.Elapsed.TotalSeconds, 2):###,##0}/sec) {folder.Name}資料夾")
        '                                End While
        '                            End Sub)
        'Await loadTask
    ...
    Private Async Function GetFolderSizeLINQ(folder As Outlook.Folder) As Task(Of Long)
        ...
        '        '        intcount += 1
        '        '        Invoke(Sub() lblStatus1.Text = intcount & " / " & GetMailCountByMAPI_Old2(folder)) '更新讀取進度
        '        '    End If
        '        'Next
        '    End Try

        '    ' 更新讀取進度
        '    'Invoke(Sub() lblStatus1.Text = intcount & " / " & GetMailCountByMAPINew(folder))    'Invoke 包 UI 更新 (Claude說不好)
        '    lblStatus1.Text = intcount & " / " & GetMailCountByMAPINew(folder)
        '    Return foldersize
        'End If
